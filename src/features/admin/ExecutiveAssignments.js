...
const ExecutiveAssignments = () => {
  ...
  const { fetchAllClients, fetchExecutivesAPI } = useApi();
  ...
  useEffect(() => {
    ...
    getAllLeads();
  }, []); // ❗ FIXED: Empty dependency → API runs only once

  /** --------------------- FIX #2 — Executive list fetch once ----------------------- **/
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const selectedIds = allClients
      .map((lead, idx) => ({ lead, index: idx + 1 }))
      .filter(({ index }) => index >= start && index <= end)
      .map(({ lead }) => String(lead.id));

    setSelectedLeads(selectedIds);
  }, [selectedRange, currentPage, allClients]);
    ...
    if (selectedExecutive) {
      ...
      if (executive) {
        filtered = filtered.filter(
          (lead) => lead.assignedToExecutive === executive.username
        );
      }
    }
  ...
  const toggleSelectAll = () => {
    setSelectedLeads(
      selectedLeads.length === leads.length ? [] : leads.map((l) => String(l.id))
    );
  };
        ...
        <div className="scrollable-container">
          <div className="leads-table">
            <div className="leads-header">
              ...
              <span className="assign-date-header">
                {filterType === "meeting" ? "Meeting Schedule Date" : "Assigned Date"}
              </span>
              <span></span>
            </div>
                    ...
                    {filterType === "meeting" ? (
                      (() => {
                        const dateTime = formatMeetingDateTime(
                          lead.meetingScheduleDate || lead.updatedAt
                        );
                        return (
